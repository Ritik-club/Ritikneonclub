      ...
      <div className={`w-full max-w-sm bg-[#1e2330] rounded-[3rem] overflow-hidden border border-white/10 shadow-[0_0_100px_rgba(0,0,0,0.8)] transition-all duration-500 transform ${show ? 'scale-100 translate-y-0' : 'scale-75 translate-y-20'} ${!isWin ? 'animate-loss-result' : ''}`}>
        ...
        <div className={`py-10 text-center relative ${isWin ? 'bg-gradient-to-b from-green-600/20 to-transparent' : 'bg-gradient-to-b from-red-600/20 to-transparent'}`}>
          <div className="absolute inset-0 flex items-center justify-center opacity-10 pointer-events-none">
            <span className="text-9xl font-black italic uppercase tracking-tighter">{isWin ? 'WIN' : 'LOSS'}</span>
          </div>
        ...
        <div className="flex flex-col items-center pb-8 px-8">
           ...
           <div className="grid grid-cols-2 gap-4 w-full mb-8">
              <div className="bg-white/5 rounded-2xl p-4 text-center border border-white/5">
                 <p className="text-[9px] text-gray-500 uppercase font-black mb-1 tracking-widest">Color</p>
              ...
              <div className="bg-white/5 rounded-2xl p-4 text-center border border-white/5">
                 <p className="text-[9px] text-gray-500 uppercase font-black mb-1 tracking-widest">Size</p>
                 <p className="text-sm font-black text-blue-400 uppercase">{result.bigSmall}</p>
              </div>
           </div>

           {isWin ? (
             <div className="w-full bg-green-500 py-4 rounded-2xl text-center shadow-xl shadow-green-900/40 animate-bounce mb-6">
                <p className="text-[10px] font-black text-white/80 uppercase tracking-widest mb-1">Total Payout</p>
                <p className="text-2xl font-black font-orbitron text-white">🪙 {userProfit.toLocaleString()}</p>
             </div>
           ) : (
             <div className="w-full bg-red-500/10 border border-red-500/20 py-4 rounded-2xl text-center mb-6">
           ...
           <button 
             ...
             className="w-full py-3 bg-white/5 border border-white/10 rounded-xl text-[10px] font-black uppercase tracking-[0.3em] text-gray-400 hover:text-white hover:bg-white/10 transition-all active:scale-95"
           >
             Close Result
           </button>
        </div>

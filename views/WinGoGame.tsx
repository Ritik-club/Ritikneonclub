  ...
  return (
    <div className="p-4 space-y-4 animate-in fade-in duration-500 bg-gray-50 min-h-full">
      <div className="bg-white rounded-[2rem] p-6 border border-gray-100 shadow-sm flex justify-between items-center">
        <div><p className="text-[10px] font-black text-gray-400 uppercase">Period ID</p><p className="text-lg font-black font-mono tracking-tighter text-red-600">{period.slice(-8)}</p></div>
      ...
      <div className="grid grid-cols-4 gap-2">
        {['30sec', '1min', '3min', '5min'].map((m: any) => (
          <button key={m} disabled={isBetPending} onClick={() => setActiveMode(m)} className={`py-3 rounded-2xl text-[10px] font-black border transition-all ${activeMode === m ? 'bg-red-600 border-red-400 text-white shadow-md' : 'bg-white border-gray-100 text-gray-400 shadow-sm'}`}>{m}</button>
        ))}
      </div>
      ...
      <div className="bg-white rounded-[2.5rem] p-6 space-y-6 shadow-sm border border-gray-100">
        {!hasAccess ? (
           <div className="text-center py-10 space-y-4">
              <div className="w-16 h-16 bg-red-50 rounded-full flex items-center justify-center text-2xl mx-auto">🔒</div>
              <p className="text-xs font-black uppercase text-gray-400">Winning Prediction Locked</p>
              <button onClick={() => setTab(Tab.WALLET)} className="w-full py-4 bg-red-600 text-white rounded-2xl font-black text-[10px] uppercase shadow-lg">Open Wallet</button>
           </div>
        ) : (
           <>
              {isBetPending && (
                <div className="bg-red-50 border border-red-100 rounded-2xl p-4 text-center">
                   <p className="text-[10px] font-black text-red-600 uppercase">Stakes Locked - Waiting Result</p>
                </div>
              )}
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <button disabled={isBetPending} onClick={() => setBetModal({type: 'Small', label: 'Small'})} className="bg-red-600 text-white py-4 rounded-2xl font-black text-sm uppercase shadow-md active:scale-95 disabled:opacity-50">Small</button>
              </div>
      ...
      {betModal && (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40 p-4 backdrop-blur-sm">
          <div className="w-full max-w-md bg-white rounded-[3rem] p-8 animate-in slide-in-from-bottom duration-300 shadow-2xl">
            <h3 className="text-xl font-black mb-6 text-gray-900">Stake on: <span className="text-red-600">{betModal.label}</span></h3>
            <div className="grid grid-cols-5 gap-2 mb-8">
              {[1, 5, 10, 50, 100].map(m => (
                <button key={m} onClick={() => setSelectedMultiplier(m)} className={`py-4 rounded-2xl font-black text-xs transition-all ${selectedMultiplier === m ? 'bg-red-600 text-white shadow-xl scale-110' : 'bg-gray-50 text-gray-400 border border-gray-100'}`}>{m}x</button>
              ))}
            </div>
            <button onClick={() => { onBet({selection: betModal.type, amount: 10 * selectedMultiplier}); setBetModal(null); }} className="w-full py-5 bg-red-600 text-white rounded-3xl font-black text-lg uppercase shadow-xl">Confirm Bet 🪙 {(10 * selectedMultiplier)}</button>
